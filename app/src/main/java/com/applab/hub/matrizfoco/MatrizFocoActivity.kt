package com.applab.hub.matrizfoco
...
import com.applab.hub.matrizfoco.data.Quadrant
import com.applab.hub.matrizfoco.data.Task
import com.applab.hub.matrizfoco.data.TaskRepository
import com.google.gson.Gson

class MatrizFocoActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.matriz_activity_main)

        val toolbar = findViewById<Toolbar>(R.id.toolbar_matriz)
        setSupportActionBar(toolbar)
        supportActionBar?.setDisplayHomeAsUpEnabled(true)
        supportActionBar?.title = "Matriz de Foco Físico"
        ...
        // Ajusta toolbar padding (status bar) e quadrant_grid margin (toolbar height)
        ...
        toolbar.addOnLayoutChangeListener { _, _, top, _, bottom, _, _, _, _ ->
            ...
            if (newHeight != toolbarHeight) {
                ...
                val quadrantGrid = findViewById<View>(R.id.quadrant_grid)
                (quadrantGrid.layoutParams as FrameLayout.LayoutParams).topMargin = toolbarHeight
                quadrantGrid.requestLayout()
                val dz = deleteZone
                (dz.layoutParams as FrameLayout.LayoutParams).apply {
                    topMargin = toolbarHeight + (10 * resources.displayMetrics.density).toInt()
                }
                dz.requestLayout()
            }
        }
    ...
    private fun addBubbleView(task: Task) {
        val bubble = layoutInflater.inflate(R.layout.matriz_item_bubble, bubbleContainer, false) as TextView
        bubble.tag = task.id
    ...
    private fun editarTarefa(taskId: String, bubble: TextView) {
        ...
        dialog.setContentView(R.layout.matriz_dialog_editar_tarefa)
    ...
    private fun mostrarAviso(mensagem: String) {
        ...
        val params = FrameLayout.LayoutParams(
            ...
            topMargin = toolbarHeight + (12 * d).toInt()
        }
